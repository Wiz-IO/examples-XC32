...
void __assert(int expr, int line, const char *file, char *msg)
{
   if (expr)
      return;
   if (NULL != msg)
      printf("ASSERT: line %d of %s, %s\n", line, file, msg);
   else
      printf("ASSERT: line %d of %s\n", line, file);
   while (1)
      continue;
}
#define ASSERT(expr) __assert((int)(expr), __LINE__, __FILE__, NULL)

#ifdef __GNUC__
#define MCU6502_JUMPTABLE
#endif /* __GNUC__ */
...
/* Warning! CPU has no decimal mode, so by default this does no BCD! */
#ifdef MCU6502_DECIMAL
...
#ifdef MCU6502_TESTOPS
#define JAM()    \
   {             \
      cpu_Jam(); \
   }
#else /* !MCU6502_TESTOPS */
#define JAM()              \
   {                       \
      PC--;                \
      cpu.jammed = true;   \
...
uint8_t mcu6502_getbyte(uint32_t address)
{
   return bank_readbyte(address);
}

/* get number of elapsed cycles */
uint32_t mcu6502_getcycles(bool reset_flag)
{
...
#ifdef MCU6502_JUMPTABLE

#define OPCODE_BEGIN(xx) op##xx:
#ifdef MCU6502_DISASM

#define OPCODE_END                                          \
   if (remaining_cycles <= 0)                               \
      goto end_execute;                                     \
   printf(mcu6502_disasm(PC, COMBINE_FLAGS(), A, X, Y, S)); \
...
#endif /* !MCU6502_DISASM */

#else /* !MCU6502_JUMPTABLE */
#define OPCODE_BEGIN(xx) case 0x##xx:
#define OPCODE_END break;
#endif /* !MCU6502_JUMPTABLE */
...
*/
int mcu6502_execute(int timeslice_cycles)
{
...
void mcu6502_burn(int cycles)
{
   cpu.burn_cycles += cycles;
}

/* Release our timeslice */
void mcu6502_release(void)
{
   remaining_cycles = 0;
}
